 ...
 * <p>
 * You can use the TypeDefinitionId annotation or getDefaultTypeDefinition() to define the type. If
 * you use the annotation, you can also register the type and create event instances with the class
 * only.
 *
...
public class MyEventType extends BaseEventTypeNode {
  ...
  /**
   * The constructor is used by the NodeBuilder and should not be used directly by the application.
   * Therefore we define it with protected visibility.
   */
  ...
  public UaVariable getMyVariableNode() {
    UaVariable property =
        (UaVariable) getComponent(new QualifiedName(getNodeManager().getNamespaceIndex(), MY_VARIABLE_NAME));
    return property;
  }

  /**
   * @param value the value to set to MyProperty
   */
  ...
  /**
   * @param value the value to set to MyVariable
   */
